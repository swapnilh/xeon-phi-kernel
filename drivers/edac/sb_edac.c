...
static const u16 sbridge_dram_rule[] = {
	0x80, 0x88, 0x90, 0x98, 0xa0,
	0xa8, 0xb0, 0xb8, 0xc0, 0xc8,
};

static const u16 ibridge_dram_rule[] = {
...
static const u16 knl_dram_rule[] = {
...
static char *show_dram_attr(u16 attr)
{
	switch (attr) {
		case 0:
			return "DRAM";
...
static const u16 sbridge_interleave_list[] = {
	0x84, 0x8c, 0x94, 0x9c, 0xa4,
	0xac, 0xb4, 0xbc, 0xc4, 0xcc,
};

static const u16 ibridge_interleave_list[] = {
...
static const u16 knl_interleave_list[] = {
...
static const u16 tad_dram_rule[] = {
	0x40, 0x44, 0x48, 0x4c,
	0x50, 0x54, 0x58, 0x5c,
	0x60, 0x64, 0x68, 0x6c,
};
				...
				if (tad_base < sad_base) {
					...
					if (tad_limit+1 > sad_limit) {
						...
						edac_dbg(3, " TAD region %d 0x%llx - 0x%llx (%lld bytes) table%d\n",
							tad_rule, tad_base, tad_limit, tad_size, mc);
		...
		for (channel = 0; channel < KNL_MAX_CHANNELS; channel++) {
			for (way = 0; way < intrlv_ways; way++) {
				...
				if (KNL_MOD3(dram_rule))
					...
					target = 0x7 & sad_pkg(pvt->info.interleave_pkg,
										interleave_reg, way);

				for (cha = 0; cha < KNL_MAX_CHAS; cha++) {
					if (knl_get_mc_route(target, mc_route_reg[cha]) == channel &&
		...
		for (channel = 0; channel < KNL_MAX_CHANNELS; channel++) {
			...
			if (participants[channel]) {
				edac_dbg(4, "mc channel %d contributes %lld bytes via sad entry %d\n",
					channel, sad_actual_size[mc]/intrlv_ways, sad_rule);
				mc_sizes[channel] += sad_actual_size[mc]/intrlv_ways;
			}
		}
	}
							...
							struct sbridge_dev *sbridge_dev)
{
	...
	for (i = 0; i < sbridge_dev->n_devs; i++) {
		...
		case PCI_DEVICE_ID_INTEL_KNL_IMC_CHA:
			...
			BUG_ON(pvt->knl.pci_cha[devidx] != NULL);
		...
		case PCI_DEVICE_ID_INTEL_KNL_IMC_CHANNEL:
			...
			BUG_ON(pvt->knl.pci_channel[devidx] != NULL);
				    ...
				    const struct mce *m)
{
	...
	if (pvt->info.type == KNIGHTS_LANDING) {
		if (channel == 14) {
			...
			snprintf(msg, sizeof(msg), "%s%s err_code:%04x:%04x channel:%d (DIMM_%c)",
				overflow ? " OVERFLOW" : "",
				(uncorrected_error && recoverable) ? " recoverable" : "",
...
static int sbridge_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	case PCI_DEVICE_ID_INTEL_BROADWELL_IMC_HA0:
	    ...
	    break;
	case PCI_DEVICE_ID_INTEL_KNL_IMC_SAD0:
		rc = sbridge_get_all_devices_knl(&num_mc, pci_dev_descr_knl_table);
